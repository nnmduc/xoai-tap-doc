...
- Image prompt files must be English only. Translate Vietnamese story details into natural English before image generation; keep Vietnamese names only as proper nouns.
- Generate only missing images unless the user asks for regeneration.
- Final story images must be generated by an AI image-generation tool/provider from the prepared prompt files, or provided explicitly by the user.
- Do not satisfy missing media with hand-written rendering code, Pillow, SVG, canvas, HTML/CSS screenshots, or placeholder art.
